...
const EmojiSlots = () => {
    ...
    const setAmountAndSpinz:SubmitHandler<z.infer<typeof EmojiSlotSchema>> = async (data) => {
        ...
        const res = await createEmojiSlot({id:v4(),amount:parseInt(data.amount),spinz:parseInt(data.spinz),createdAt:new Date().toISOString(),profileId:profile?.id,currentAmount:0,currentSpin:0,currentEmojis:emojisArray.toString()})
        if(res.data){
    ...
    const handleSpin = async () => {
        ...
        if(currentspinCount<spinz){
            ...
            if(!emojiSlotFromAppState || !currentEmojis) {console.log("emojiSlotFromAppState doesn't exist");return;};
